...
# 读取数据集
print('user_info')
user_info_train = pd.read_csv('../data/train/user_info_train_2.csv')
user_info_test = pd.read_csv('../data/test/user_info_test_2.csv')
# 合并train、test
...
user_info.to_csv('../data/part2/data1.csv' , index = False , encoding="utf-8" , mode='a')
...
user_info.to_csv('../data/part2/data2.csv' , index = False , encoding="utf-8" , mode='a')
...
for i in user_info['user_id'].unique():
    ...
    for j in data_2['browse_data']:
        user_info.loc[user_info['user_id'] == i , 'bef_loan_browse_data_' + str(j)] = user_info.loc[user_info['user_id'] == i , 'bef_loan_browse_data_' + str(j)] + 1
    for j in data_2['child_numbers']:
        user_info.loc[user_info['user_id'] == i , 'bef_loan_child_numbers_' + str(j)] = user_info.loc[user_info['user_id'] == i , 'bef_loan_child_numbers_' + str(j)] + 1
               
user_info.to_csv('../data/part2/data3.csv' , index = False , encoding="utf-8" , mode='a')
...
'''这个文件的特征分为3类，每一类都是先添加特征（加初始化），再进行赋值'''
'''每一部分特征都是同时进行 借款时间之前 和 之后 一块来'''
# 先是第一部分
# 1：每一个用户有多少张账单
# 2：多上种银行（unique）
# 3：还款状态有几种 (unique)
...
user_info.to_csv('../data/part2/data4.csv' , index = False , encoding="utf-8" , mode='a')
...
user_info.to_csv('../data/part2/data5.csv' , index = False , encoding="utf-8" , mode='a')
